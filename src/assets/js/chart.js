var createHierarchyChart = (function() {
    return {
        init: function(data) {
        
            var params = {
                selector: "#svgChart",
                chartWidth: window.innerWidth - 270,
            ...
            function drawOrganizationChart(params) {
                ...
                function update(source, param) {
                    ...
                    .attr("x", 125 + dynamic.nodeTextLeftMargin )
                    .attr("y", attrs.nodePadding + 4)
                    .attr('class', 'emp-count-icon')
                    .attr("text-anchor", "right")
                    .style('font-family', 'FontAwesome')
                    .on("click", function(d, id){
                        console.log(d, "dhfdshhdsfhdhdskjhfkd", id, d.id);
                        if (d.id === id + 1){
                            nodeGroup.append("rect")
                            .attr("class", 'chart-dropdown')
                            .attr("width", 60)
                            .attr("height", 60)
                            .attr("x", attrs.nodeWidth)
                            .attr("y", 0)
                            .text(function(v){
                                for (let i = 0; i < d.select.length; i++) {
                                    nodeGroup.append("text")
                                    .style("fill", "red")
                                    .attr("x", attrs.nodeWidth)
                                    .attr("y", attrs.nodeHeight - 70)
                                    .text(function(d){
                                        return d.select[i].action
                                    })
                                }
                               }   
                                )
                    ...
                    .text(function(d) {
                        if (d.children || d._children) return attrs.ellipsisIcon;
                    });

                    nodeGroup.append("text")
                        .attr("x", dynamic.nodeTextLeftMargin)
                        .attr("y", attrs.nodePadding + 15) 
                        .attr('class', 'emp-name')
                        .attr("text-anchor", "right")
                        .text(function(d) {
                            return d.name.trim();
                        ...
                        .attr('class', 'object-icon')
                        .attr('y', 2 + attrs.nodePadding)                       
                        .attr('x', + attrs.nodePadding)
                        .attr('preserveAspectRatio', 'none')
                        .attr('width', dynamic.nodeImageWidth -1)
                        .attr('height', dynamic.nodeImageHeight - 3)
                    ...
                    nodeGroup.append("text")
                        ...
                        .attr("text-anchor", "right")
                        .style('font-family', 'FontAwesome')
                        .attr("x", dynamic.nodeTextLeftMargin + 20)
                ...
                function reflectResults(results) {
                    var htmlStringArray = results.map(function(result) {
                        ...
                        strVar += "              <img class=\"image\" src=\"" + result. img+ "\"\/>";
                        strVar += "            <\/div>";
                ...
                function back() {
                    ...
                    show(['.btn-action']);
                    hide([ '.btn-action.btn-back', '.department-information'])
                    clear(params.selector);
